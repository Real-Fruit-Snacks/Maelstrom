...
def create_parser():
    ...
    parser = argparse.ArgumentParser(
        ...
        description=textwrap.dedent("""
            ╔═══════════════════════════════════════════════════════════════════╗
            ║  Maelstrom - NetExec AD Enumeration with enum4linux-ng Style      ║
            ╚═══════════════════════════════════════════════════════════════════╝
        ...
        epilog=textwrap.dedent("""
            ...
            Examples:
              ...
              Anonymous enumeration (auto-probes null/guest sessions):
                maelstrom 10.0.24.230

              Single credential:
                maelstrom 10.0.24.230 -u admin -p 'Password123' -d CORP
                maelstrom 10.0.24.230 -u admin -H <ntlm_hash>

              Multi-credential (compare access levels):
                maelstrom 10.0.24.230 -C creds.txt -d CORP
                maelstrom 10.0.24.230 -U users.txt -P passes.txt

              Multi-target (CIDR, ranges, files):
                maelstrom 10.0.0.0/24 -u admin -p pass          # CIDR notation
                maelstrom 10.0.0.1-50 -u admin -p pass          # IP range
                maelstrom 10.0.0.1-10.0.0.50 -u admin -p pass   # Full range
                maelstrom targets.txt -u admin -p pass          # Targets file (auto-detected)

              Specific modules:
                maelstrom 10.0.24.230 -u admin -p pass --shares --users
                maelstrom 10.0.24.230 -u admin -p pass --delegation --adcs
                maelstrom 10.0.24.230 -u admin -p pass --laps --ldap-signing

              Other protocols:
                maelstrom 10.0.24.230 -u admin -p pass --mssql    # MSSQL databases
                maelstrom 10.0.24.230 -u admin -p pass --rdp      # RDP/NLA status
                maelstrom 10.0.24.230 --ftp                       # FTP anonymous (no auth)
                maelstrom 10.0.24.230 --nfs                       # NFS exports (no auth)

              Output options:
                maelstrom 10.0.24.230 -u admin -p pass -o results.txt
                maelstrom 10.0.24.230 -u admin -p pass -j -o results.json
