...
All notable changes to the Maelstrom project will be documented in this file.
...
### Changed
- **Renamed package** from `nxc_enum` to `maelstrom` across all modules and documentation.
- **pipx install support** - Can now install via `pipx install git+https://github.com/Real-Fruit-Snacks/maelstrom.git`.
- **Removed redundant files** - Removed `setup.py`, `setup.cfg`, `requirements.txt`, `requirements-dev.txt`, and `maelstrom.py` standalone wrapper. All configuration is now in `pyproject.toml`.

## [1.12.1] - 2026-02-04

### Fixed
- **Formatting** - Fixed automated formatting issues to satisfy CI requirements.
...
- **Kerberos Authentication** - Full Kerberos authentication support
  ...
  - Example: `maelstrom dc01.corp.local -u admin --use-kcache`
  - Example: `maelstrom dc01.corp.local -u admin --aesKey <key> --kdcHost dc01.corp.local`
- **Certificate Authentication** - PKINIT certificate-based authentication
  ...
  - Example: `maelstrom dc01.corp.local -u admin --pfx-cert admin.pfx --pfx-pass certpass`
- **Local Authentication** (`--local-auth`) - Authenticate against local SAM instead of domain
  ...
  - Example: `maelstrom 192.168.1.100 -u Administrator -p pass --local-auth`
- **Kerberos Delegation** - S4U2proxy and S4U2self delegation support
  ...
  - Example: `maelstrom dc01.corp.local -u svc_sql -p pass --delegate admin --self`
- **Spray Control Options** - Fine-grained control over credential testing
  ...
  - Example: `maelstrom 10.0.0.1 -U users.txt -p 'Summer2024!' --jitter 2 --fail-limit 10`
...
- **Local Groups Filter** (`--local-groups-filter`) - Filter to specific local group
  - Example: `maelstrom 10.0.0.1 -u admin -p pass --local-groups --local-groups-filter Administrators`
- **Custom LDAP Query** (`--query`, `--query-attrs`) - Execute custom LDAP queries
  ...
  - Example: `maelstrom 10.0.0.1 -u admin -p pass --query '(objectClass=computer)' --query-attrs cn,operatingSystem`
...
- **Proxy Mode** (`--proxy-mode`) - Optimize for proxychains/SOCKS operation
  ...
  - Example: `proxychains maelstrom 10.0.0.1 -u user -p pass`
- **Validate-Only Mode** (`--validate-only`) - Fast credential testing without enumeration
  ...
  - Example: `maelstrom 10.0.0.1 -C creds.txt --validate-only`
...
- **Discover-Only Mode** (`--discover-only`) - Quick reconnaissance without enumeration
  ...
  - Example: `maelstrom 10.0.0.0/24 --discover-only`
...
- **Module Exports Completed** - Added 9 missing exports to `__init__.py`
  ...
  - All new modules now properly importable from `maelstrom.enums`
...
- **Multi-Target Scanning** - Scan multiple targets with CIDR notation, IP ranges, or target files
  - CIDR notation support: `maelstrom 10.0.0.0/24 -u admin -p pass`
  - IP range support: `maelstrom 10.0.0.1-50 -u admin -p pass`
  - Full IP range: `maelstrom 10.0.0.1-10.0.0.50 -u admin -p pass`
  - Target file support: `maelstrom targets.txt -u admin -p pass` (auto-detected!)
...
- **Revamped README** - Complete rewrite with improved structure and clarity
  - Added "Why Maelstrom?" problem/solution table
...
[Unreleased]: https://github.com/Real-Fruit-Snacks/maelstrom/compare/v1.8.0...HEAD
[1.8.0]: https://github.com/Real-Fruit-Snacks/maelstrom/compare/v1.7.0...v1.8.0
[1.7.0]: https://github.com/Real-Fruit-Snacks/maelstrom/compare/v1.6.0...v1.7.0
[1.6.0]: https://github.com/Real-Fruit-Snacks/maelstrom/compare/v1.5.1...v1.6.0
[1.5.1]: https://github.com/Real-Fruit-Snacks/maelstrom/compare/v1.5.0...v1.5.1
[1.5.0]: https://github.com/Real-Fruit-Snacks/maelstrom/compare/v1.4.0...v1.5.0
[1.4.0]: https://github.com/Real-Fruit-Snacks/maelstrom/compare/v1.3.0...v1.4.0
[1.3.0]: https://github.com/Real-Fruit-Snacks/maelstrom/compare/v1.2.0...v1.3.0
[1.2.0]: https://github.com/Real-Fruit-Snacks/maelstrom/compare/v1.1.0...v1.2.0
[1.1.0]: https://github.com/Real-Fruit-Snacks/maelstrom/compare/v1.0.0...v1.1.0
[1.0.0]: https://github.com/Real-Fruit-Snacks/maelstrom/releases/tag/v1.0.0
